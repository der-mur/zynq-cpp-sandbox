...
int addTtc0ToInterruptSystem(TtcSingleTimer* p_TtcSingleTimer0_0)
{
	...
	status = XScuGic_Connect(p_XScuGicInst, TTC0_INT_IRQ_ID,
				  (Xil_ExceptionHandler) p_TtcSingleTimer0_0->getUserIntrHandler(),
				  (void *) p_TtcSingleTimer0_0);
	if (status != XST_SUCCESS)
	{
		return XST_FAILURE;
	}
...
int addUart1ToInterruptSystem(PsUart* p_PsUart1)
{

	int status;

	// Connect a device driver handler for Uart1
	status = XScuGic_Connect(&XScuGicInst, UART1_INTR_ID,
				  (Xil_ExceptionHandler) p_PsUart1->getUserIntrHandler(),
				  (void *) p_PsUart1);
	if (status != XST_SUCCESS)
	{
		return XST_FAILURE;
	...
	XScuGic_Enable(p_XScuGicInst, UART1_INTR_ID);
